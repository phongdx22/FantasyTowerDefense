	...
	public class Unit : MonoBehaviour
	{
		...
		*/
		...
		IEnumerator EnterGhostMode()
		{
			...
			// Optional: Make player semi-transparent
			movement.playerSprite.color = new Color(1f, 1f, 1f, 0.5f);
			movement.buildUI.SetActive(false);
			...
			canBeAttacked = true;
			canBeTargeted = true;
			movement.isGhost = false;
			movement.buildUI.SetActive(true);

			// Restore appearance
			movement.playerSprite.color = movement.originalColor;
		...
		public void TakeTroopDamage(int damageTaken) //take damage dealt by troop
		{
			if (!canBeAttacked)
			{
				TDTK.TextOverlay("immuned", GetTargetPoint());
			...
			{
				TDTK.TextOverlay("+" + damageTaken.ToString(), GetTargetPoint());
		...
		{   //not in used, not in live package
			string text = "";
