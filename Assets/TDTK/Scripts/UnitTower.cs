...
namespace TDTK
{
	...
	public class UnitTower : Unit
	{
		...
		*/
		...
		[Space(10)]
		[Tooltip("Use in Free-Form mode only, specify the space occupied by the tower")]
		...
		public List<float> GetUpgradeCost(int upgradeIdx = 0)
		{
			...
			{ //return upgradeTowerList[upgradeIdx].GetCost();
		...
		public override void Destroyed(bool spawnEffDestroyed = true, bool destroyedByAttack = true)
		{
			...
			if (this.name == "MeleeTroop(Clone)" || this.name == "RangedTroop(Clone)" || this.name == "AOETroop(Clone)")
			{
				Debug.Log("Troop has died, respawning");
